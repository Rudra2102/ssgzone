...
router.get('/tenants', superAdminAuth, async (req, res) => {
  try {
    const result = await db.query(`
      SELECT 
        tc.id, tc.company_name, tc.company_slug, tc.admin_name, tc.admin_email,
        tc.max_users, tc.status, tc.created_at,
      ...
      GROUP BY tc.id, tc.company_name, tc.company_slug, tc.admin_name, tc.admin_email,
               tc.max_users, tc.status, tc.created_at, sa.saas_name
...
router.get('/email/templates', superAdminAuth, async (req, res) => {
  try {
    const result = await db.query('SELECT id, name, subject, html_content as body, category, is_active, created_at, updated_at FROM email_templates ORDER BY created_at DESC');
...
router.post('/email/templates', superAdminAuth, async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO email_templates (tenant_id, name, subject, html_content, category) VALUES ('00000000-0000-0000-0000-000000000000', $1, $2, $3, $4) RETURNING id, name, subject, html_content as body, category, created_at`,
...
router.put('/email/templates/:id', superAdminAuth, async (req, res) => {
  try {
    const { name, subject, body, category } = req.body;
    const result = await db.query(
      `UPDATE email_templates SET name=$1, subject=$2, html_content=$3, category=$4, updated_at=NOW() WHERE id=$5 RETURNING id, name, subject, html_content as body, category, updated_at`,
...
router.get('/branding', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: result.rows[0] || { platform_name: 'SSGzone', primary_color: '#4f46e5' } });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.put('/branding', superAdminAuth, async (req, res) => {
  try {
    ...
    const result = await db.query(
      `UPDATE platform_branding SET platform_name=$1, primary_color=$2, secondary_color=$3, updated_at=NOW()
       WHERE id=(SELECT id FROM platform_branding ORDER BY id LIMIT 1) RETURNING *`,
      [platform_name, primary_color, secondary_color]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
...
router.patch('/admins/:id/reset-password', superAdminAuth, async (req, res) => {
  try {
    ...
    const result = await db.query(
      'UPDATE platform_admins SET password_hash=$1, updated_at=NOW() WHERE id=$2 RETURNING id, username',
